...
- Username: `admin`
- Password: `admin` (override in env for non-local usage)
...
- `JARVIS_RUNTIME_ROOT` should point to the external SSD root you mount into Podman.
- For architecture debugging, check `/api/v1/status` fields: `ingress`, `tool_isolation`, `orchestrator.artifact_store`.
