...
def test_delivery_config_defaults() -> None:
    ...
    assert cfg.delivery.deploy_max_retries >= 0
...
def test_delivery_config_env_overrides(monkeypatch) -> None:
    ...
    monkeypatch.setenv("JARVIS_DELIVERY_DEPLOY_MAX_RETRIES", "3")
    monkeypatch.setenv("JARVIS_DELIVERY_DEPLOY_RETRY_BACKOFF_SECONDS", "2")
    ...
    assert cfg.delivery.deploy_max_retries == 3
    assert cfg.delivery.deploy_retry_backoff_seconds == 2.0
